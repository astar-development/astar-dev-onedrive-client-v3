...
public class AccountRepositoryShould
{
    ...
    public async Task AddNewAccountSuccessfully()
    {
        using var context = CreateInMemoryContext();
        var repository = new AccountRepository(context);
        var account = new AccountInfo("acc1", "John Doe", @"C:\Sync1", true, null, null, false, false);
    ...
    public async Task GetAllAccountsCorrectly()
    {
        using var context = CreateInMemoryContext();
        var repository = new AccountRepository(context);
        await repository.AddAsync(new AccountInfo("acc1", "User 1", @"C:\Sync1", true, null, null, false, false));
        await repository.AddAsync(new AccountInfo("acc2", "User 2", @"C:\Sync2", false, null, null, false, false));
    ...
    public async Task GetAccountByIdCorrectly()
    {
        using var context = CreateInMemoryContext();
        var repository = new AccountRepository(context);
        await repository.AddAsync(new AccountInfo("acc1", "User 1", @"C:\Sync1", true, null, "token123", false, false));
    ...
    public async Task UpdateExistingAccountSuccessfully()
    {
        using var context = CreateInMemoryContext();
        var repository = new AccountRepository(context);
        await repository.AddAsync(new AccountInfo("acc1", "Old Name", @"C:\Sync1", true, null, null, false, false));

        var updated = new AccountInfo("acc1", "New Name", @"C:\NewPath", false, DateTime.UtcNow, "newToken", false, false);
    ...
    public async Task ThrowExceptionWhenUpdatingNonExistentAccount()
    {
        using var context = CreateInMemoryContext();
        var repository = new AccountRepository(context);
        var account = new AccountInfo("nonexistent", "Name", @"C:\Path", true, null, null, false, false);
    ...
    public async Task DeleteAccountSuccessfully()
    {
        using var context = CreateInMemoryContext();
        var repository = new AccountRepository(context);
        await repository.AddAsync(new AccountInfo("acc1", "User", @"C:\Sync", true, null, null, false, false));
